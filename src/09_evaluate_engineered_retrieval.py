...
BASELINE_DB_DIR = "chroma_baseline"
BASELINE_COLLECTION_NAME = "officeqa_baseline"

ENGINEERED_DB_DIR = "chroma_engineered"
ENGINEERED_COLLECTION_NAME = "officeqa_engineered"
...
baseline_client = chromadb.PersistentClient(path=BASELINE_DB_DIR)

baseline_collection = baseline_client.get_collection(
    name=BASELINE_COLLECTION_NAME,
    embedding_function=embedding_fn
)

engineered_client = chromadb.PersistentClient(path=ENGINEERED_DB_DIR)

engineered_collection = engineered_client.get_collection(
    name=ENGINEERED_COLLECTION_NAME,
    embedding_function=embedding_fn
)
...
for _, row in tqdm(questions_df.iterrows(), total=len(questions_df)):
    ...
    collection=engineered_collection,
    question=question,
...
results_df.to_csv("results/engineered_retrieval_results.csv", index=False)

print("\nSaved: results/engineered_retrieval_results.csv")